...
class PredictionPipeline:
    ...
    def save_input_files(self)-> str:
        ...
        try:
            ...
            input_csv_file = self.request.files['file']  # it collect the specific file which is upload in the browser
            pred_file_path = os.path.join(pred_file_input_dir, input_csv_file.filename)   # input_csv_file.filename: This extracts the original filename of the uploaded CSV file.

            input_csv_file.save(pred_file_path)   # It is used to save the file to a specified location which means Saving the Uploaded File.
